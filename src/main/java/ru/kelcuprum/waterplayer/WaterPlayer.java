...
//            log("tick");
            ...
            while (loadTrack.consumeClick()) {
                if (!WaterPlayer.clothConfig) {
                    client.getToasts().addToast(new ControlToast(Localization.getText("waterplayer.message.clothConfigNotFound"), true));
                    return;
                }
                client.setScreen(MusicScreen.buildScreen(client.screen));
            }
        });
    ...
    public static void start() {
        TIMER.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                if(closing) return;
                if(UserConfig.ENABLE_CHANGE_TITLE && WaterPlayer.music != null && music.getAudioPlayer().getPlayingTrack() != null) updateTitle();
                if (UserConfig.ENABLE_BOSS_BAR) updateBossBar();
